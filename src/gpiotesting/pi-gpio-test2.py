...
#   GPIO.setmode(GPIO.BCM)     # Use BCM-style   numbering
    ...
    GPIO.setup(11, GPIO.IN, pull_up_down=GPIO.PUD_DOWN)  # Read from pin 11

    GPIO.output(12, GPIO.HIGH)  # Set pin 12 high  (on / True / 1)

    while True:                          # Listen forever for something
        print("I'm waiting...")
        if GPIO.input(11) == GPIO.HIGH:  # Heard something...
            print("Pin 11 now high")
            GPIO.output(12, GPIO.LOW)    # Set pin 12 low (off / False / 0)
            break                        # Stop listening

    GPIO.cleanup()                       # Put the pins back to a known state
